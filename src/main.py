...
# 4) If authentication failed, exit immediately unless running in local mode.
# LOCAL_MODE=true is for developer machines using 'az login' where the first
# App Configuration auth probe can be transiently flaky; the app continues and
# individual connectors still authenticate normally on first real use.
_local_mode = os.getenv("LOCAL_MODE", "false").lower() in ("true", "1", "yes")
if getattr(cfg, "auth_failed", False):
    msg = "The orchestrator is not authenticated (run 'az login' or configure Managed Identity)."
    if _local_mode:
        logging.warning(msg + " Continuing startup (LOCAL_MODE=true).")
    else:
